...
mod kernels {
    ...
    pub unsafe fn embedding_lookup_f32(
        ...
        let table_base: PointerTile<*mut f32, { [] }> = pointer_to_tile(table);
        let table_base: PointerTile<*mut f32, { [1] }> = table_base.reshape(const_shape![1]);
        let table_ptrs: PointerTile<*mut f32, { [128] }> = table_base.broadcast(tile_shape);
        let table_ptrs: PointerTile<*mut f32, { [128] }> = table_ptrs.offset_tile(source_offsets);
