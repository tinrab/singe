...
mod kernels {
    ...
    #[cutile::entry()]
    pub unsafe fn gather_f32(out: *mut f32, input: *mut f32, indices: *mut u32, len: i32) {
        let pid: (i32, i32, i32) = get_tile_block_id();
        let tile_shape = const_shape![128];
        let offsets: Tile<i32, { [128] }> =
            iota(tile_shape) + broadcast_scalar(pid.0 * 128i32, tile_shape);
    ...
    #[cutile::entry()]
    pub unsafe fn gather_row_f32(out: *mut f32, input: *mut f32, row_index: i32, cols: i32) {
        let pid: (i32, i32, i32) = get_tile_block_id();
        let tile_shape = const_shape![128];
        let offsets: Tile<i32, { [128] }> =
            iota(tile_shape) + broadcast_scalar(pid.0 * 128i32, tile_shape);
    ...
    #[cutile::entry()]
    pub unsafe fn scatter_rows_f32(
        out: *mut f32,
        input: *mut f32,
        row_indices: *mut u32,
        cols: i32,
        ...
        let out_base: PointerTile<*mut f32, { [1] }> = out_base.reshape(const_shape![1]);
        let out_ptrs: PointerTile<*mut f32, { [128] }> = out_base.broadcast(tile_shape);
        let out_ptrs: PointerTile<*mut f32, { [128] }> = out_ptrs.offset_tile(dest_offsets);
        store_ptr_tko(
            out_ptrs,
            values.0,
            ordering::Weak,
    ...
    #[cutile::entry()]
    pub unsafe fn scatter_f32(out: *mut f32, input: *mut f32, indices: *mut u32, len: i32) {
        let pid: (i32, i32, i32) = get_tile_block_id();
        let tile_shape = const_shape![128];
        let offsets: Tile<i32, { [128] }> =
            iota(tile_shape) + broadcast_scalar(pid.0 * 128i32, tile_shape);
    ...
    #[cutile::entry()]
    pub unsafe fn copy_indexed_rows_f32(
        out: *mut f32,
        input: *mut f32,
        ...
        let out_base: PointerTile<*mut f32, { [1] }> = out_base.reshape(const_shape![1]);
        let out_ptrs: PointerTile<*mut f32, { [128] }> = out_base.broadcast(tile_shape);
        let out_ptrs: PointerTile<*mut f32, { [128] }> = out_ptrs.offset_tile(output_offsets);
        store_ptr_tko(
            out_ptrs,
            values.0,
            ordering::Weak,
