...
mod kernels {
    ...
    pub unsafe fn fmha_prefill_lse_mma_f32<const BN: i32, const D: i32>(
        out: *mut f32,
        lse: &mut Tensor<f32, { [1, 16] }>,
        query: &Tensor<f32, { [-1, -1, -1, -1] }>,
        ...
        // Grid x packs batch and query head.
        // Grid y selects a 16-query tile.
        // Grouped-query attention maps multiple Q heads to one KV head.
        let heads = query.shape()[2];
        let batch_index = pid.0 / heads;
        let head = pid.0 - batch_index * heads;
        let query_tile = pid.1;
        let kv_head = head / query_group_size;
        let key_len = key.shape()[1];
        let key_tiles = ceil_div(key_len, BN);

        let two: Tile<f32, { [] }> = constant(2.0f32, const_shape![]);
        let log2: f32 = tile_to_scalar(log(two));
        let scale_log2: Tile<f32, { [16, BN] }> =
            broadcast_scalar(scale / log2, const_shape![16, BN]);

        // Online softmax state.
        // `row_max` and `row_sum` track the stable normalization over all visited K tiles.
        // `acc` keeps the unnormalized weighted sum of V.
        let mut row_max: Tile<f32, { [16, 1] }> = constant(f32::NEG_INFINITY, const_shape![16, 1]);
        let mut row_sum: Tile<f32, { [16, 1] }> = constant(0.0f32, const_shape![16, 1]);
        let mut acc: Tile<f32, { [16, D] }> = constant(0.0f32, const_shape![16, D]);

        // Load the resident Q tile once.
        // K/V are streamed one BN-column tile at a time below.
        let query_part = query.partition(const_shape![1, 16, 1, D]);
        let query_tile_values: Tile<f32, { [16, D] }> = query_part
            .load([batch_index, query_tile, head, 0i32])
            .reshape(const_shape![16, D]);

        let key_part = key.partition(const_shape![1, BN, 1, D]);
        let value_part = value.partition(const_shape![1, BN, 1, D]);

        let query_offsets: Tile<i32, { [16] }> =
            iota(const_shape![16]) + broadcast_scalar(query_tile * 16, const_shape![16]);
        let query_offsets = query_offsets.reshape(const_shape![16, 1]);
        let query_offsets: Tile<i32, { [16, BN] }> = query_offsets.broadcast(const_shape![16, BN]);
        let key_offsets_base: Tile<i32, { [BN] }> = iota(const_shape![BN]);
        let key_offsets_base: Tile<i32, { [16, BN] }> = key_offsets_base
            .reshape(const_shape![1, BN])
            .broadcast(const_shape![16, BN]);
        let key_len_tile: Tile<i32, { [16, BN] }> = broadcast_scalar(key_len, const_shape![16, BN]);
        let mask_true: Tile<f32, { [16, BN] }> = constant(0.0f32, const_shape![16, BN]);
        let mask_false: Tile<f32, { [16, BN] }> = constant(f32::NEG_INFINITY, const_shape![16, BN]);

        for key_tile in 0i32..key_tiles {
            // Compute QK^T for this K tile.
            // Padded keys and future keys are masked before converting to exp2 scale.
            let key_values: Tile<f32, { [BN, D] }> = key_part
                .load([batch_index, key_tile, kv_head, 0i32])
                .reshape(const_shape![BN, D]);
            let key_values_t: Tile<f32, { [D, BN] }> = key_values.transpose();
            let mut scores: Tile<f32, { [16, BN] }> = constant(0.0f32, const_shape![16, BN]);
            scores = mma(query_tile_values, key_values_t, scores);

            let key_offsets: Tile<i32, { [16, BN] }> =
                broadcast_scalar(key_tile * BN, const_shape![16, BN]) + key_offsets_base;
            let mut valid = lt_tile(key_offsets, key_len_tile);
            if causal != 0i32 {
                valid = valid & ge_tile(query_offsets, key_offsets);
            }
            scores = scores + select(valid, mask_true, mask_false);
            scores = scores * scale_log2;

            // Merge the current tile into the running online softmax.
            // When the row max increases, rescale the existing row sum and accumulator.
            let tile_max: Tile<f32, { [16] }> = reduce_max(scores, 1);
            let tile_max: Tile<f32, { [16, 1] }> = tile_max.reshape(const_shape![16, 1]);
            let next_max = max_tile(row_max, tile_max);
            let centered = scores - next_max.broadcast(const_shape![16, BN]);
            let probabilities: Tile<f32, { [16, BN] }> = exp2(centered, ftz::Disabled);
            let tile_sum: Tile<f32, { [16] }> = reduce_sum(probabilities, 1);
            let tile_sum: Tile<f32, { [16, 1] }> = tile_sum.reshape(const_shape![16, 1]);
            let alpha: Tile<f32, { [16, 1] }> = exp2(row_max - next_max, ftz::Disabled);
            row_sum = row_sum * alpha + tile_sum;
            acc = acc * alpha.broadcast(const_shape![16, D]);

            let value_values: Tile<f32, { [BN, D] }> = value_part
                .load([batch_index, key_tile, kv_head, 0i32])
                .reshape(const_shape![BN, D]);
            acc = mma(probabilities, value_values, acc);
            row_max = next_max;
        }

        // Finish softmax normalization.
        // Store columns manually because the output layout is provided as raw strides rather than a Tensor view.
        let normalized = true_div(acc, row_sum.broadcast(const_shape![16, D]));
        let output_query_offsets: Tile<i32, { [16] }> =
            iota(const_shape![16]) + broadcast_scalar(query_tile * 16, const_shape![16]);
        let query_valid = lt_tile(
            output_query_offsets,
            broadcast_scalar(query.shape()[1], const_shape![16]),
        );
        for dim in 0i32..D {
            let zero_scalar: Tile<i32, { [] }> = constant(0i32, const_shape![]);
            let dim_scalar: Tile<i32, { [] }> = scalar_to_tile(dim);
            let column: Tile<f32, { [16, 1] }> = extract(normalized, [zero_scalar, dim_scalar]);
            let column: Tile<f32, { [16] }> = column.reshape(const_shape![16]);
            let output_offsets =
                broadcast_scalar(batch_index * output_batch_stride, const_shape![16])
                    + output_query_offsets
                        * broadcast_scalar(output_sequence_stride, const_shape![16])
                    + broadcast_scalar(head * output_head_stride + dim, const_shape![16]);
            store_vector_16(out, output_offsets, column, query_valid);
        }

        // Save log-sum-exp in natural-log units for callers.
        // `row_max` is tracked in log2 units.
        // `row_sum` is the unlogged sum of exp2(score - row_max) terms.
        let lse_values = row_max * broadcast_scalar(log2, const_shape![16, 1]) + log(row_sum);
        lse.store(lse_values.reshape(const_shape![1, 16]));
    }

    #[cutile::entry()]
    pub unsafe fn fmha_prefill_lse_f32_hd4(
        out: *mut f32,
        lse: *mut f32,
        query: *mut f32,
        key: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_lse_f32_hd8(
        out: *mut f32,
        lse: *mut f32,
        query: *mut f32,
        key: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_lse_f32_hd16(
        out: *mut f32,
        lse: *mut f32,
        query: *mut f32,
        key: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_lse_f32_hd32(
        out: *mut f32,
        lse: *mut f32,
        query: *mut f32,
        key: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_lse_f32_hd64(
        out: *mut f32,
        lse: *mut f32,
        query: *mut f32,
        key: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_lse_f32_hd128(
        out: *mut f32,
        lse: *mut f32,
        query: *mut f32,
        key: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_lse_f32_hd256(
        out: *mut f32,
        lse: *mut f32,
        query: *mut f32,
        key: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_f32_hd4(
        out: *mut f32,
        query: *mut f32,
        key: *mut f32,
        value: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_f32_hd8(
        out: *mut f32,
        query: *mut f32,
        key: *mut f32,
        value: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_f32_hd16(
        out: *mut f32,
        query: *mut f32,
        key: *mut f32,
        value: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_f32_hd32(
        out: *mut f32,
        query: *mut f32,
        key: *mut f32,
        value: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_f32_hd64(
        out: *mut f32,
        query: *mut f32,
        key: *mut f32,
        value: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_f32_hd128(
        out: *mut f32,
        query: *mut f32,
        key: *mut f32,
        value: *mut f32,
    ...
    #[cutile::entry()]
    pub unsafe fn fmha_prefill_f32_hd256(
        out: *mut f32,
        query: *mut f32,
        key: *mut f32,
        value: *mut f32,
    ...
    fn sliding_window_attention_f32<const HEAD_DIM: i32, const WINDOW: i32>(
        ...
        // This fallback maps one vector lane to one output scalar.
        // The flat output offset is decoded into batch, query token, attention head, and head-dimension coordinates.
        // The same kernel covers every HEAD_DIM.
    ...
    fn fmha_prefill_lse_f32<const HEAD_DIM: i32>(
        ...
        // Pass 1 computes the per-output maximum score.
        // This is the standard softmax stabilization term.
        // It also applies the causal mask when requested.
        let mut max_score = constant(-1.0e20f32, tile_shape);
        for key_index in 0i32..key_len {
            let key_valid = valid
                & if causal != 0 {
        ...
        let mut denom = constant(0.0f32, tile_shape);
        let mut sum = constant(0.0f32, tile_shape);
        // Pass 2 recomputes scores and subtracts the max.
        // It accumulates the softmax denominator plus one value dimension for this output scalar.
        for key_index in 0i32..key_len {
            let key_valid = valid
                & if causal != 0 {
                    cmpi(
        ...
        // LSE is shared by all dimensions of the same query/head row.
        // Only the first head-dimension lane writes it to avoid duplicate stores.
