...
mod kernels {
    ...
    pub fn binary_store<const OP: i32, const B: i32>(
        ...
        let lhs_tile: Tile<f64, { [B] }> = load_tile_like(lhs, out);
        let rhs_tile: Tile<f64, { [B] }> = load_tile_like(rhs, out);
        let value = if OP == 0 {
            lhs_tile + rhs_tile
        } else if OP == 1 {
            lhs_tile - rhs_tile
            ...
            let neg_inf = constant(f64::NEG_INFINITY, out.shape());
    ...
    #[cutile::entry()]
    pub unsafe fn dequantize_u8_f64(
        out: *mut f64,
        input: *mut u8,
        scale: f64,
        zero_point: f64,
        ...
        let input_f64: Tile<f64, { [128] }> = convert_tile(input_result.0);
        let values = (input_f64 - broadcast_scalar(zero_point, tile_shape))
    ...
    #[cutile::entry()]
    pub unsafe fn dequantize_i8_f64(
        out: *mut f64,
        input: *mut i8,
        scale: f64,
        zero_point: f64,
    ...
    pub fn reduce_impl<const OP: i32, const BN: i32>(
        out: &mut Tensor<f64, { [1, 1] }>,
        input: &Tensor<f64, { [-1, -1] }>,
        divisor: f64,
    ) {
        let pid: (i32, i32, i32) = get_tile_block_id();
        let tile_shape = const_shape![1i32, BN];
        let input_tile_raw: Tile<f64, { [1, BN] }> =
        ...
        let value: Tile<f64, { [1] }> = if OP == 0 || OP == 1 {
            ...
            let divisor = broadcast_scalar(divisor, const_shape![1]);
            let sum: Tile<f64, { [1] }> = reduce_sum(input_tile, 1i32);
            let mean: Tile<f64, { [1] }> = sum / divisor;
            ...
            let variance: Tile<f64, { [1] }> = variance_sum / divisor;
            if OP == 6 {
            ...
            let fill = constant(f64::NEG_INFINITY, tile_shape);
            reduce_max(select(valid_cols, input_tile_raw, fill), 1i32)
        } else if OP == 3 {
            let fill = constant(f64::INFINITY, tile_shape);
    ...
    pub fn arg_reduce_impl<const OP: i32, const BN: i32>(
        ...
        let candidates = if OP == 0 {
            let fill: Tile<f64, { [1, BN] }> = constant(f64::NEG_INFINITY, tile_shape);
            select(valid_cols & numeric, input_tile_raw, fill)
        } else {
            let fill: Tile<f64, { [1, BN] }> = constant(f64::INFINITY, tile_shape);
            select(valid_cols & numeric, input_tile_raw, fill)
        };
        let best_value: Tile<f64, { [1] }> = if OP == 0 {
    ...
    >(
        out: &mut Tensor<f64, { [BM, BN] }>,
        input: &Tensor<f64, { [-1, -1] }>,
    ) {
        let input_tile_raw: Tile<f64, { [BM, BN] }> = load_tile_like(input, out);
        let zero = constant(0.0f64, out.shape());
        ...
        let valid_cols = cmpi(col_iota, cols_tile, predicate::LessThan);
        let negative_infinity = constant(f64::NEG_INFINITY, out.shape());
        let input_tile = select(valid_cols, input_tile_raw, negative_infinity);
        let input_max: Tile<f64, { [BM] }> = reduce_max(input_tile, 1i32);
        let input_max = input_max
            .reshape(const_shape![BM, 1i32])
            .broadcast(out.shape());
        let numerator = exp(input_tile - input_max);
        let denominator: Tile<f64, { [BM] }> = reduce_sum(numerator, 1i32);
    ...
    #[cutile::entry()]
    pub fn softmax_lse_f64<const BN: i32>(
        out: &mut Tensor<f64, { [1, 1] }>,
        input: &Tensor<f64, { [-1, -1] }>,
    ) {
        let tile_shape = const_shape![1i32, BN];
        let input_tile_raw: Tile<f64, { [1, BN] }> = input
        ...
        let input_tile = select(
            valid_cols,
            input_tile_raw,
            constant(f64::NEG_INFINITY, tile_shape),
        );
        let input_max: Tile<f64, { [1] }> = reduce_max(input_tile, 1i32);
        let input_max_broadcast = input_max
            .reshape(const_shape![1i32, 1i32])
            .broadcast(tile_shape);
        let numerator = exp(input_tile - input_max_broadcast);
        let denominator: Tile<f64, { [1] }> = reduce_sum(numerator, 1i32);
