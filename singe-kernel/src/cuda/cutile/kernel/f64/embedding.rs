...
mod kernels {
    ...
    #[cutile::entry()]
    pub unsafe fn embedding_lookup_f64(
        out: *mut f64,
        token_ids: *mut u32,
        table: *mut f64,
        width: i32,
        ...
        let table_base: PointerTile<*mut f64, { [] }> = pointer_to_tile(table);
        let table_base: PointerTile<*mut f64, { [1] }> = table_base.reshape(const_shape![1]);
        let table_ptrs: PointerTile<*mut f64, { [128] }> = table_base.broadcast(tile_shape);
        let table_ptrs: PointerTile<*mut f64, { [128] }> = table_ptrs.offset_tile(source_offsets);
