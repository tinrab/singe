...
mod kernels {
    ...
    #[cutile::entry()]
    pub unsafe fn gather_f16(out: *mut f16, input: *mut f16, indices: *mut u32, len: i32) {
        let pid: (i32, i32, i32) = get_tile_block_id();
        let tile_shape = const_shape![128];
        let offsets: Tile<i32, { [128] }> =
            iota(tile_shape) + broadcast_scalar(pid.0 * 128i32, tile_shape);
    ...
    #[cutile::entry()]
    pub unsafe fn gather_row_f16(out: *mut f16, input: *mut f16, row_index: i32, cols: i32) {
        let pid: (i32, i32, i32) = get_tile_block_id();
        let tile_shape = const_shape![128];
        let offsets: Tile<i32, { [128] }> =
            iota(tile_shape) + broadcast_scalar(pid.0 * 128i32, tile_shape);
    ...
    #[cutile::entry()]
    pub unsafe fn scatter_rows_f16(
        out: *mut f16,
        input: *mut f16,
        row_indices: *mut u32,
        cols: i32,
        ...
        let out_base: PointerTile<*mut f16, { [1] }> = out_base.reshape(const_shape![1]);
        let out_ptrs: PointerTile<*mut f16, { [128] }> = out_base.broadcast(tile_shape);
        let out_ptrs: PointerTile<*mut f16, { [128] }> = out_ptrs.offset_tile(dest_offsets);
        store_ptr_tko(
            out_ptrs,
            values.0,
            ordering::Weak,
    ...
    #[cutile::entry()]
    pub unsafe fn scatter_f16(out: *mut f16, input: *mut f16, indices: *mut u32, len: i32) {
        let pid: (i32, i32, i32) = get_tile_block_id();
        let tile_shape = const_shape![128];
        let offsets: Tile<i32, { [128] }> =
            iota(tile_shape) + broadcast_scalar(pid.0 * 128i32, tile_shape);
    ...
    #[cutile::entry()]
    pub unsafe fn copy_indexed_rows_f16(
        out: *mut f16,
        input: *mut f16,
        ...
        let out_base: PointerTile<*mut f16, { [1] }> = out_base.reshape(const_shape![1]);
        let out_ptrs: PointerTile<*mut f16, { [128] }> = out_base.broadcast(tile_shape);
        let out_ptrs: PointerTile<*mut f16, { [128] }> = out_ptrs.offset_tile(output_offsets);
        store_ptr_tko(
            out_ptrs,
            values.0,
            ordering::Weak,
