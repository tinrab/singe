...
use cutile::{cuda_async::device_operation::DeviceOp, cuda_core::Stream};
...
#[allow(dead_code)]
trait DeviceOpExt: DeviceOp + Sized {
    fn enqueue_on(self, stream: &Arc<Stream>) -> cutile::error::Result<<Self as DeviceOp>::Output> {
        // Safety: callers own stream ordering and keep raw device allocations alive until the enqueued work has completed.
        unsafe { self.async_on(stream) }.map_err(cutile::error::Error::from)
    }
}
...
impl<T> DeviceOpExt for T where T: DeviceOp {}
