use singe_cuda_find::{
    ...
    emit_static_link_libraries, find_cuda_runtime, find_cutensor, find_nccl,
};
...
fn link_libraries() {
    ...
    if emit_dependency_metadata(find_cutensor()).is_some() {
        emit_link_libraries(&["cutensor", "cutensorMg", "cutensorMp"]);
    }
