...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let a = DeviceMemory::from_slice(&[f8e4m3::from_bits(1); (M * K) as usize])?;
    let b = DeviceMemory::from_slice(&[f8e4m3::from_bits(1); (K * N) as usize])?;
