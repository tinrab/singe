...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let a_batches = [
        DeviceMemory::from_slice(&[f8e4m3::from_bits(1); (M * K) as usize])?,
        DeviceMemory::from_slice(&[f8e4m3::from_bits(1); (M * K) as usize])?,
    ];
    let b_batches = [
        DeviceMemory::from_slice(&[f8e4m3::from_bits(1); (K * N) as usize])?,
        DeviceMemory::from_slice(&[f8e4m3::from_bits(1); (K * N) as usize])?,
    ];
    let c_batches = [
    ...
    let scale_a_tensors = [
        DeviceMemory::from_slice(&[1.0_f32])?,
        DeviceMemory::from_slice(&[1.0_f32])?,
    ];
    let scale_b_tensors = [
        DeviceMemory::from_slice(&[1.0_f32])?,
    ...
    let a_layout = MatrixLayout::create_grouped(DataType::F8E4M3, &rows, &reduction, &lda)?;
    let b_layout = MatrixLayout::create_grouped(DataType::F8E4M3, &reduction, &cols, &ldb)?;
    ...
    desc.set_a_scale_mode(MatrixScale::PerBatchScalar32F)?;
    desc.set_b_scale_mode(MatrixScale::PerBatchScalar32F)?;
    ...
    println!("lt_fp8gemm_grouped_simple groups: {}", rows.len());
    Ok(())
}
