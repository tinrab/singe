...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let a_batches = [
        DeviceMemory::from_slice(&vec![f8e4m3::from_bits(1); (M * K) as usize])?,
        DeviceMemory::from_slice(&vec![f8e4m3::from_bits(1); (M * K) as usize])?,
    ];
    let b_batches = [
        DeviceMemory::from_slice(&vec![f8e4m3::from_bits(1); (K * N) as usize])?,
        DeviceMemory::from_slice(&vec![f8e4m3::from_bits(1); (K * N) as usize])?,
    ];
    let c_batches = [
    ...
    desc.set_a_scale_mode(MatrixScale::Block128x12832F)?;
    desc.set_b_scale_mode(MatrixScale::Block128x12832F)?;
    ...
    println!(
        "lt_blk128x128_fp8gemm_grouped_simple groups: {}",
        rows.len()
    );
    Ok(())
}
