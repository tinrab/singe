...
fn main() -> Result<()> {
    println!("cuDSS example: solving with an optional threading layer");
    ...
    if let Some(path) = std::env::args()
        .nth(1)
        .or_else(|| std::env::var("CUDSS_THREADING_LIB").ok())
    {
        println!("threading layer: {path}");
        context.set_threading_layer(&path)?;
    } else {
        println!("threading layer: default cuDSS execution");
    }
